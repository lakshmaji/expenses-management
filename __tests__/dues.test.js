...
describe("House Dues Management", () => {
    describe('DUES', () => {
        ...
        it('should not have any dues when no one else have spent any amount', () => {
            ...
            expect(result).toEqual([{ amount: 0, from: FAKE_NAMES.TANGLED }])
        });

        it('should have some dues when some one else have spent amount', () => {
            addMembers(house, [FAKE_NAMES.PANDA, FAKE_NAMES.GRU])
            spendWithRoommates(house, [[TESTING_CONSTANTS.AMOUNTS.D, FAKE_NAMES.GRU, FAKE_NAMES.PANDA]])
        ...
        it('should print dues by amount in descending order', () => {
            addMembers(house, [FAKE_NAMES.SNOWBALL, FAKE_NAMES.MINION, FAKE_NAMES.SUPER_RHINO])
            spendWithRoommates(house, [
                [TESTING_CONSTANTS.AMOUNTS.L, FAKE_NAMES.MINION, FAKE_NAMES.SNOWBALL],
                [TESTING_CONSTANTS.AMOUNTS.N, FAKE_NAMES.SUPER_RHINO, FAKE_NAMES.SNOWBALL]
            ])
        ...
        it('should print no dues when there are no dues', () => {
            addMembers(house, [FAKE_NAMES.FOR_THE_BIRDS, FAKE_NAMES.GRU, FAKE_NAMES.SUPER_RHINO])
            spendWithRoommates(house, [[TESTING_CONSTANTS.AMOUNTS.B, FAKE_NAMES.GRU, FAKE_NAMES.FOR_THE_BIRDS]])
        ...
        it('should print dues by name in ascending order when amounts are equal', () => {
            addMembers(house, [FAKE_NAMES.DRU, FAKE_NAMES.WALL_E, FAKE_NAMES.SUPER_RHINO])
            spendWithRoommates(house, [
